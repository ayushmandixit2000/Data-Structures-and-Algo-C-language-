   ...
   while(i<SIZE && revInfix[i]!='\0')
    {
        ...
        case '-':
            ...
            push(&s,c);
            break;
        case ')':                           //Special: If the operator is ')', then push it into the stack.
            push(&s,c);
            break;
        case '(':                          //If the operator is '(', then pop all the operators from the stack till it finds ')' opening bracket in the stack.
            while(isEmptyStack(s)==0)     //Reason for the change: after the reverse,  every '(' becomes ')' and every ')' becomes '('.
            {
                if(peek(s)!=')')
                {
                    revPrefix[j++]=peek(s);
